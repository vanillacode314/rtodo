...
export default function FolderPage() {
	...
	const [appContext, { setCurrentNode }] = useApp();

	const nodes = createQuery(() =>
		queries.nodes.byPath({ path: appContext.path, includeChildren: true })
	);

	async function updateNode(data: Partial<schema.TNode>) {
		await createMessages(schema.nodes, data);
						...
						<DropdownMenuItem
							onSelect={() => {
								confirmModal.open({
									...
									onYes: () => deleteFile().catch(() => toast.error('Something went wrong')),
									title: 'Delete File'
								});
							}}
						>
		...
		<Node
			...
			dropdownMenu={
				<DropdownMenu>
					<DropdownMenuTrigger
						as={Button<'button'>}
						class="h-full rounded-none can-hover:invisible group-hover:can-hover:visible"
						size="icon"
						variant="ghost"
					>
						<span class="i-heroicons:ellipsis-vertical text-lg" />
					...
					<DropdownMenuContent class="w-48">
						...
						<DropdownMenuItem
							onSelect={() => {
								confirmModal.open({
									message: `Are you sure you want to delete ${props.node.name}?`,
									onYes: () => deleteFolder().catch(() => toast.error('Something went wrong')),
									title: 'Delete Folder'
								});
							}}
						>
...
function Node(props: {
	...
	return (
		<div class={cn('relative flex overflow-hidden bg-white/5', props.class)} ref={props.ref}>
