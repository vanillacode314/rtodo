...
const DatePickerInput = (props: DatePickerPrimitive.InputProps) => {
	...
	return (
		<DatePickerPrimitive.Input
			class={cn(
				'border-border bg-background placeholder:text-muted-foreground focus-visible:ring-ring h-9 w-full border px-3 py-1 text-sm shadow-sm transition-shadow focus-visible:ring-[1.5px] focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50',
				local.class
			)}
			{...others}
		/>
...
const DatePickerTrigger = (props: DatePickerPrimitive.TriggerProps) => {
	...
	return (
		<DatePickerPrimitive.Trigger
			class={cn(
				'border-border bg-background hover:bg-accent/50 focus-visible:ring-ring flex min-h-9 min-w-9 items-center justify-center border transition-[box-shadow,background-color] focus-visible:ring-[1.5px] focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>svg]:size-4',
				local.class
			)}
			{...others}
		>
...
const DatePickerContent = (props: DatePickerPrimitive.ContentProps) => {
	...
	return (
		<DatePickerPrimitive.Content
			class={cn(
				'bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 z-50 border p-3 shadow-md outline-none',
				local.class
			)}
			{...others}
		>
...
const DatePickerTableHeader = (props: DatePickerPrimitive.TableHeaderProps) => {
	...
	return (
		<DatePickerPrimitive.TableHeader
			class={cn('text-muted-foreground w-8 flex-1 text-[0.8rem] font-normal', local.class)}
			{...others}
		/>
	);
};
...
const DatePickerTableCell = (props: DatePickerPrimitive.TableCellProps) => {
	...
	return (
		<DatePickerPrimitive.TableCell
			class={cn(
				'has-[[data-in-range]]:bg-accent has-[[data-outside-range][data-in-range]]:bg-accent/50 flex-1 p-0 text-center text-sm',
				local.class
			)}
			{...others}
		/>
