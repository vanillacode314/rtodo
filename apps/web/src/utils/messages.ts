...
async function applyUpdate(
	...
	switch (update.operation) {
		case 'insert': {
			...
			await tx.query({
				sql: `
          INSERT INTO "${getTableName(update.table)}"("id",${columns.map((column) => `"${column}"`).join(',')},"createdAt","updatedAt")
		...
		case 'update': {
			for (const column of columns) {
				await tx.query({
					sql: `UPDATE "${getTableName(update.table)}" SET "${column}"=? WHERE "id"=? AND ("updatedAt"->>'$.${column}'<? OR "updatedAt"->>'$.${column}' IS NULL)`,
					params: [update.data[column], id, timestamp].map((value) => toSql(value))
				});
				await tx.query({
					sql: `UPDATE "${getTableName(update.table)}" SET "updatedAt"=json_replace("updatedAt", '$.${column}', ?) WHERE "id"=?`,
