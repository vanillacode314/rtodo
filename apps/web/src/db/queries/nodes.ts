...
import { nodes, nodesSchema, TNode } from '~/db/schema';
...
export async function getNodeByPath(
	...
	return throwOnParseError(nodesSchema.array()(await runCustomQuery(query)));
}

export async function getPathByNodeId(id: string) {
	...
	const [path] = (await db.get<{ path: string }>(query)) as unknown as [string];
